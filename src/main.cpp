...
public:
	...
	void render(GLFWwindow *window)
	{
		...
		if (mouseDown)
		{
			...
			Phi = (posY / height) * 3.14159 + (3.14159 / 2);
			Theta = (posX / width) * 2 * 3.14159 + 3.14159;
		}
		...
		View->pushMatrix();
			View->lookAt(vec3(0.0, 0.0, 0.0), vec3(cos(Phi) * cos(Theta), sin(Phi), cos(Phi) * sin(Theta)), vec3(0.0, 1.0, 0.0));
